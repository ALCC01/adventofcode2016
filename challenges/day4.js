...
function solve(input, challenge) {
    if (challenge === 1) {
        ...
        return input.split`\n`
            .map(e => e.toUpperCase().split`-`.map((e, i, a) => {
                if (i < a.length - 1) {
                    ...
                    var c = Object.keys(letters).sort((a, b) => {
                        if (letters[a] - letters[b] != 0) return letters[b]-letters[a];
                        else return (a < b) ? -1 : (a > b) ? 1 : 0;
                    }),
                        sector = e.slice(0, 3),
                        chksum = e.slice(4, 9);
                    letters = {};
                    if (c.join``.startsWith(chksum)) return +sector;
                    else return 0;
                }
        ...
        return input.split`\n`
            ...
            .filter(e => e.startsWith('NORTHPOLE OBJECT STORAGE'))
            .join``
    }
}
